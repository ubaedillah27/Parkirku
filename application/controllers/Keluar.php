...
class Keluar extends CI_Controller {
	...
	public function index(){
		...
		$data['keluar'] = $this->db->query("SELECT * FROM tbl_keluar RIGHT JOIN tbl_masuk ON tbl_keluar.kd_masuk = tbl_masuk.kd_masuk WHERE tgl_jam_keluar LIKE '".date('Y-m-d')."%' AND status_keluar = '1'")->result_array();
		// die(print_r($data));
